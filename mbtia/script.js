...
function showIntroPage(pageNum) {
    ...
    if (currentIntroPage > 0) {
        document.getElementById(`intro-page-${currentIntroPage}`).style.display = 'none';
    }
    
    // 새로운 소개 페이지 표시
    document.getElementById(`intro-page-${pageNum}`).style.display = 'block';
    currentIntroPage = pageNum;
}

// 테스트 시작
function startTest() {
    // 마지막 소개 페이지 숨기기
    document.getElementById('intro-page-3').style.display = 'none';
...
function shareResult() {
    ...
    const shareTitle = `나의 MBTI는 ${mbtiType}입니다!`;
    const shareDesc = `${resultTitle}\n\n${resultDesc.substring(0, 100)}...`;
        ...
        if (window.Kakao) {
            window.Kakao.Link.sendDefault({
                ...
                content: {
                    ...
                    imageUrl: 'https://testpro.site/mbtia/MBTI.png',
                    link: {
                        mobileWebUrl: 'https://testpro.site/mbtia/',
                        webUrl: 'https://testpro.site/mbtia/'
                    },
                },
                buttons: [
                    {
            ...
            alert('카카오톡이 설치되어 있지 않거나 모바일 기기가 아닙니다.');
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async function() {
    // 결과 확인
    if (window.location.hash === '#result-section') {
        const mbtiResult = sessionStorage.getItem('mbtiResult');
        const storedScores = sessionStorage.getItem('mbtiScores');
        
        if (mbtiResult && storedScores) {
            mbtiScores = JSON.parse(storedScores);
            document.getElementById('start-section').style.display = 'none';
            document.getElementById('result-section').style.display = 'block';
            document.getElementById('mbti-result').textContent = mbtiResult;
            
            // 결과 데이터 로드
            await loadLanguageData();
            const resultData = window.mbtiResults[mbtiResult];
            if (resultData) {
                document.getElementById('result-title').textContent = resultData.title;
                document.getElementById('result-description').textContent = resultData.description;
                displayStrengthsWeaknesses(resultData);
                displayCompatibility(resultData);
                displayFamousPeople(resultData);
                displayTraitBars(mbtiResult);
            }
        }
    }
    // 테스트 시작 파라미터 확인
    const shouldStartTest = sessionStorage.getItem('startTest');
    if (shouldStartTest === 'true') {
        sessionStorage.removeItem('startTest'); // 파라미터 제거
        document.getElementById('start-section').style.display = 'none';
        document.getElementById('question-section').style.display = 'block';
        startTest();
    }
