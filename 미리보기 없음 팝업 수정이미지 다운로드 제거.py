...
def scrape_category():
    ...
    # 사용자 설정 확인
    print("\n스크래핑 설정:")
    print("1. 이미지/동영상 게시물은 건너뜁니다")
    print("2. 게시물을 10개씩 스크래핑합니다")
    ...
    try:
        ...
        while True:
            ...
            for article in articles:
                try:
                    # 이미지/동영상 게시물 필터링
                    if (article.select_one('.entry-content img') or 
                        article.select_one('.entry-content iframe')):
                        logging.info("Skipping image/video post")
                        continue
